...
class HybridMLXUNet:
    ...
    def patch_pipeline(pipeline, model_path: str, weights_path: Optional[str] = None):
        ...
        class _MLXUNetProxy(torch.nn.Module):
            """Expose Hunyuan metadata without retaining duplicate Torch weights."""
            ...
            def __init__(self):
                ...
                for name in (
                    ...
                    if hasattr(original_unet, name):
                        setattr(self, name, getattr(original_unet, name))
                for token in getattr(original_unet, "pbr_setting", ()):
                    name = f"learned_text_clip_{token}"
                    if hasattr(original_unet, name):
                        value = getattr(original_unet, name).detach()
                        self.register_buffer(name, value, persistent=False)
                self._mlx_hybrid = hybrid

            def forward(self, *args, **kwargs):
                return self._mlx_hybrid.forward(*args, **kwargs)

        pipeline.unet = _MLXUNetProxy()
        del original_unet
        gc.collect()
        if torch.backends.mps.is_available():
            torch.mps.empty_cache()

        print("[MLX] Pipeline patched — duplicate Torch UNet released; forward uses MLX")
        return hybrid
